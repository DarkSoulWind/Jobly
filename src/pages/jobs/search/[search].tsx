...
import type { JobListing } from "src/lib/scraper/scraper";
import type { SavedJob } from "@prisma/client";
import {
  JobSearchReducer,
  JobSearchState,
  JOB_SEARCH_ACTION,
} from "src/lib/reducers/jobReducer";
...
import JobDistanceFilters from "src/components/listbox/JobDistanceFilters";
import JobResults from "src/components/job/JobResults";
...
const SelectedPreview = lazy(() =>
  import("src/components/job/SelectedPreview").then((module) => ({
...
const initState = (
  ...
  { searchInput, locationInput }: { searchInput: string; locationInput: string }
): JobSearchState => {
...
const Jobs: NextPage<InferGetServerSidePropsType<typeof getServerSideProps>> = (
  ...
  const [selectedJob, setSelectedJob] = useState({
    ...
    link: props.jobResults?.results ? props.jobResults?.results[0]?.link : "",
    type: props.jobResults?.results ? props.jobResults?.results[0]?.type : "",
  });
  ...
  const {
    ...
    fetchNextPage,
    hasNextPage,
  } = trpc.job.infiniteJobs.useInfiniteQuery(
    {
    ...
    <>
      ...
      <div className="p-2 flex flex-col items-center">
        ...
        <div className="w-full text-2xl flex justify-center py-5">
          <h4>
            {isFetchingJobData || isFetchingNextJobPage || !jobData ? (
              <span className="animate-pulse">Searching jobs...</span>
            ) : (
              // number of jobs present
              jobData.pages.flatMap((page) => page.results).length > 0 && (
                <>
                  Showing{" "}
                  <strong>
                    {jobData!.pages.flatMap((page) => page.results).length}
                  </strong>{" "}
                  results for{" "}
              ...
              {jobTypes.map((jobType, index) => (
                ...
                >
                  <input
                    type="checkbox"
                    name={jobType}
                    value={jobType.toLowerCase().split(" ").join("-")}
          ...
          {selectedJob && (
            <Suspense fallback={<SkeletonLoadingJobPreview />}>
              <SelectedPreview
                ...
                isSaved={
                  !!savedJobData?.find(
                    (savedJob) => savedJob.link === selectedJob.link
                  )
                }
                isErrorJobData={
                  isErrorJobData ||
                  jobData?.pages[jobData?.pages.length - 1].results.length === 0
                }
                router={router}
                jobData={jobData}
              />
...
async function getJobs(
  ...
  const response = await axios.get<any, AxiosResponse<JobsAPIResponse, Error>>(
    url
  );
  ...
  console.log("Current cursor", cursor, "next cursor", responseData.nextCursor);
  return responseData;
}
