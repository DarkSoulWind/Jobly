...
import {
  ...
  UserWithPreferences,
  FEED_ACTION,
} from "src/lib/reducers/feedReducer";
import PostComponent from "src/components/post/Post";
import { useSession } from "next-auth/react";
import { useRouter } from "next/router";
import Alert from "src/components/alert/Alert";
import SkeletonLoaderPost from "src/components/post/SkeletonLoaderPost";
...
const Feed: NextPage = () => {
  ...
  const [createPostModalOpen, , toggleCreatePostModal] = useModal(false);
  ...
  } = trpc.post.getRecommended.useQuery(
    {
  ...
  return (
    <>
              ...
              <div className="flex flex-col items-center">
                ...
                <div className="pt-9 pb-4 text-center">
                  <h3 className="font-bold text-md">
                    ...
                    {yourData?.preferences?.firstName ?? yourData?.name}{" "}
                    {yourData?.preferences?.lastName ?? ""}
                  </h3>
                  <h4 className="text-sm text-slate-500">
                    {yourData?.preferences?.bio ?? "Empty bio"}
                  </h4>
                  <button
                    onClick={() => router.push(`/user/${yourData?.name}`)}
          ...
          <div className="w-[30rem] flex flex-col gap-2 items-center justify-start">
                ...
                <div className="w-full mt-3 flex justify-center items-center gap-3">
                  <button
                    onClick={() => {
                      ...
                      localStorage.setItem("lookingForAJob", "false");
                    }}
            ...
            <div className="flex flex-col w-full gap-2">
              {postsFetchLoading &&
                new Array(3)
                  .fill(1)
                  .map((_i, index) => <SkeletonLoaderPost key={index} />)}
      ...
      <Toaster containerClassName="shadow-none" />
